...
function bld(
  ...
  return {
    ...
    fragment: HDR + body, uniforms: [],
    params,
    defaults: { speed: 1, intensity: 1, distortion: 1, scale: 1, brightness: 1, hueShift: 0, saturation: 1 },
...
export function genReactive(): ShaderDefinition[] {
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const extra = [`float g=beatGate(1.0)+beat*2.0;`, `float colr=fract(p.x*float(${n})+t*0.5+${s}*1.5);`]
    ...
    out.push(bld(`tr-gate-${k + 1}`, 'Techno Gate ' + (k + 1), 'vj', `Hard strobing beat-gated color walls (${s})`, body(extra, main),
      [{ id: 'cols', label: 'Columns', min: 2, max: 16, def: 6, step: 1, group: 'shape' }],
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const extra = [`float rr=length(p)*(1.0-${s}*0.3);`, `float r0=rr-(uBeatPhase*0.4+${s}*0.5);`, `float ring=exp(-abs(fract(r0*float(${n}))-0.5)*7.0)*beatGate(1.0);`]
    ...
    out.push(bld(`tr-kickring-${k + 1}`, 'Kick Rings ' + (k + 1), 'cosmic', `Beat-triggered expanding rings driven by ${s}`, body(extra, main),
      [{ id: 'rings', label: 'Rings', min: 2, max: 16, def: 6, step: 1, group: 'shape' }],
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const main = `pal(aa/6.28*float(${n})+rr*2.0+${s}*2.0+t*0.4,${h})*(exp(-rr*1.2)+0.2*bass)`
    out.push(bld(`tr-rot-${k + 1}`, 'Hypno Tunnel ' + (k + 1), 'abstract', `Hypnotic rotating tunnel (trance staple) on ${s}`, body(extra, main),
      [{ id: 'arms', label: 'Arms', min: 2, max: 16, def: 6, step: 1, group: 'shape' }],
  ...
  for (let k = 0; k < 10; k++) {
    const h = H[k], s = SIG[k], n = 4 + (k % 5)
    const extra = [`p+=vec2(noise(p*2.0+t*0.4),noise(p*2.0-t*0.4))*distortion;`, `float sw=sin(length(p)*float(${n})-t*2.0+atan(p.y,p.x)*3.0)+${s}*1.5;`]
    ...
    out.push(bld(`tr-swirl-${k + 1}`, 'Spectral Swirl ' + (k + 1), 'liquid', `Swirling spectral liquid reacting to ${s}`, body(extra, main),
      [{ id: 'freq', label: 'Frequency', min: 2, max: 12, def: 5, step: 0.5, group: 'shape' }],
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const extra = [`float a=atan(p.y,p.x);float rr=length(p);`, `p=mat2(cos(t*0.3+uBeatPhase*0.6),-sin(t*0.3+uBeatPhase*0.6),sin(t*0.3+uBeatPhase*0.6),cos(t*0.3+uBeatPhase*0.6))*p;`, `float rays=sin(a*float(${n})+t*2.0)*0.5+0.5;`, `float ring=pow(max(0.0,sin(rr*8.0-t*3.0+${s}*4.0)),3.0);`]
    ...
    out.push(bld(`tr-mandala-${k + 1}`, 'Rave Mandala ' + (k + 1), 'geometric', `Rotating rave mandala pulsing with ${s}`, body(extra, main),
      [{ id: 'petals', label: 'Petals', min: 3, max: 20, def: 8, step: 1, group: 'shape' }],
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const extra = [`float amp=0.15+${s}*0.5;`, `float w=sin(p.x*float(${n})+t*2.0)+${s}*1.4*sin(p.x*float(${n})*0.7-t*3.0);`, `float d=exp(-abs(p.y-w*amp)*10.0);`]
    ...
    out.push(bld(`tr-basswave-${k + 1}`, 'Bass Visor ' + (k + 1), 'synthwave', `Saw-wave visor oscilloscope driven by ${s}`, body(extra, main),
      [{ id: 'freq', label: 'Frequency', min: 2, max: 16, def: 8, step: 1, group: 'shape' }],
      [{ signal: sig2signal(s), param: 'intensity', amount: 0.6, curve: 'log' }]))
  }
  ...
  for (let k = 0; k < 10; k++) {
    const h = H[k], s = SIG[k]
    const extra = [`p+=vec2(noise(p*2.0+t),noise(p*2.0-t))*distortion;`, `float f=sin(p.x*3.0+t)+sin(p.y*4.0-t*0.8)+fbm(p*2.0+${s}*3.0)+${s}*2.0;`]
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const extra = [`vec2 id=floor(p*float(${n}));vec2 fr=fract(p*float(${n}))-0.5;`, `float rnd=hash(id+floor(t*2.0));`, `float sp=exp(-length(fr)*5.0)*step(0.6,rnd)*(0.3+0.9*beat);`]
    ...
    out.push(bld(`tr-starburst-${k + 1}`, 'Starburst ' + (k + 1), 'particle', `Strobing particle starfield gated by ${s}`, body(extra, main),
      [{ id: 'stars', label: 'Stars', min: 4, max: 20, def: 10, step: 1, group: 'shape' }],
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const extra = [`float a=atan(p.y,p.x);float rr=length(p);`, `float sp=mod(a/6.28*float(${n})-log(rr+0.05)*1.6+t*0.6+${s}*0.5,1.0);`, `float arm=smoothstep(0.3,0.0,abs(fract(sp*float(${n}))-0.5));`]
    ...
    out.push(bld(`tr-galaxy-${k + 1}`, 'Spiral Galaxy ' + (k + 1), 'cosmic', `Hypnotic spiral galaxy wound louder by ${s}`, body(extra, main),
      [{ id: 'arms', label: 'Arms', min: 2, max: 10, def: 4, step: 1, group: 'shape' }],
      [{ signal: sig2signal(s), param: 'distortion', amount: 0.5, curve: 'log' }]))
  }
  ...
  for (let k = 0; k < 10; k++) {
    ...
    const extra = [`float a=atan(p.y,p.x);float rr=length(p);`, `float seg=acos(cos(a*float(${n})));`, `float rad=rr+${s}*0.3+uBeatPhase*0.15;`, `float ed=exp(-abs(rad-0.35-fbm(vec2(seg,rr)*3.0+t)*0.25)*40.0);`]
    ...
    out.push(bld(`tr-prism-${k + 1}`, 'Chroma Prism ' + (k + 1), 'geometric', `Chromatic prism edge with ${s}-driven motion`, body(extra, main),
      [{ id: 'sides', label: 'Sides', min: 3, max: 12, def: 6, step: 1, group: 'shape' }],
      [{ signal: sig2signal(s), param: 'distortion', amount: 0.5, curve: 'log' }]))
  }
  ...
  for (let k = 0; k < 10; k++) {
    const h = H[k], s = SIG[k], n = 4 + (k % 5)
    const extra = [`vec2 c=floor(p*float(${n}));`, `float check=mod(c.x+c.y+floor(t*4.0)+${s}*4.0,2.0);`, `float gl=exp(-abs(length(fract(p*float(${n}))-0.5))*6.0);`]
    ...
    out.push(bld(`tr-checker-${k + 1}`, 'Boggle Checker ' + (k + 1), 'vj', `Flashing checkerboard grid gated to ${s}`, body(extra, main),
      [{ id: 'cells', label: 'Cells', min: 2, max: 12, def: 6, step: 1, group: 'shape' }],
      [{ signal: sig2signal(s), param: 'intensity', amount: 0.6, curve: 'log' }]))
  }
  ...
  for (let k = 0; k < 10; k++) {
    ...
    out.push(bld(`tr-orbital-${k + 1}`, 'Orbital Bass ' + (k + 1), 'particle', `Orbiting bass-pumped glow bodies reacting to ${s}`, body2,
      [{ id: 'bodies', label: 'Bodies', min: 3, max: 12, def: 6, step: 1, group: 'shape' }],
